...
const Lighting = () => {
  ...
  // Νέες states για την φόρμα προσθήκης
  ...
  useEffect(() => {
    // Mock data για αρχική λίστα (αντί για backend GET)
    const mockLights = [
  ...
  const toggleLight = async (deviceId: string) => {
    ...
    try {
      // Εδώ normally θα καλούσες το backend, για demo αλλάζει μόνο local state
  ...
  // Handlers για τη φόρμα προσθήκης
  const handleAddDeviceClick = () => {
    setShowAddForm(true);
    setError("");
  };

  const handleAddDeviceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleAddDeviceSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Προσθέτουμε το νέο device στη λίστα φωτιστικών
      setLights([...lights, {...data, brightness: 100, status: false, category: "lighting" }]);
    ...
    <div className="min-h-screen bg-gradient-to-b from-muted/30 to-background pb-20">
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-3 pt-2">
          {lights
            .filter((light) => light.name.toLowerCase().includes(searchQuery.toLowerCase()))
            .map((light) => (
            <div
              key={light.id}
        ...
        {showAddForm && (
          <form 
            className="mt-4 p-4 bg-white rounded shadow-card flex flex-col gap-2" 
          ...
          >
            ...
            />
            <div>
              <Button type="submit" disabled={isAdding}>Προσθήκη</Button>
              <Button type="button" onClick={() => setShowAddForm(false)} className="ml-2">Άκυρο</Button>
            </div>
