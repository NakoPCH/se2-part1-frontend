...
// 1. GET URL FROM ENV
// This reads the variable we loaded in cypress.config.ts.
// If it finds nothing (e.g. CI without env), it defaults to localhost:5050.
const API_BASE_URL = Cypress.env('VITE_API_BASE_URL') || 'http://localhost:5050';
...
describe('End-to-End Flow: Login to Lighting Control', () => {
    ...
    it('starts at Welcome, logs in, navigates to Lighting, and manages a device', () => {
        ...
        // UPDATED: Uses the dynamic API_BASE_URL + /api prefix
        cy.request({
            method: 'POST',
            url: `${API_BASE_URL}/api/lighting/devices`,
            body: {
        ...
        // Check if the switch is actually ON
        cy.contains('.bg-white', TEST_DEVICE_NAME)
            .find('button[role="switch"]')
